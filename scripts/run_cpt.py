...
def main():
    ...
    with training_args.main_process_first(desc="Log a few random samples from the processed training set"):
        for index in random.sample(range(len(train_dataset)), 4):
            logger.info(f"Sample {index} of the processed training set:\n\n{train_dataset[index][data_args.text_column]}")
    ...
    ###############
    logger.info("*** Train ***")
    logger.info(f"Original training dataset size: {len(train_dataset)}")
    checkpoint = None
